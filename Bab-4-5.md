...
Nilai P@3 dan P@5 keduanya sempurna (1,0000), konsisten dengan NDCG@3 = 1,0000 dan NDCG@5 = 1,0000. Kueri C04 menunjukkan kondisi ideal di mana seluruh dokumen yang dikembalikan relevan sekaligus tersusun dalam urutan yang optimal, sehingga dua aspek yang diukur oleh NDCG dan Precision secara bersamaan terpenuhi.
...
Dari perspektif **pengurangan halusinasi** yang merupakan Tujuan 1 penelitian ini, hasil *Black Box Testing* mengonfirmasi bahwa arsitektur RAG AudioMatch bekerja sesuai rancangan. Pada seluruh skenario pengujian, respons LLM hanya merujuk pada produk dan informasi yang benar-benar tersedia dalam basis pengetahuan Rendy Audio. Tidak ada produk yang difabrikasi atau spesifikasi teknis yang dikarang oleh model. Skenario 4 ("resep nasi goreng") secara khusus membuktikan bahwa sistem menolak pertanyaan di luar domain tanpa menghasilkan informasi yang tidak berdasar, melainkan secara eksplisit menyatakan keterbatasan cakupan sistem. Mekanisme ini bekerja karena *system prompt* AudioMatch secara eksplisit membatasi ruang jawaban LLM pada konteks yang diambil oleh komponen retrieval, sehingga model tidak dapat mengisi kekosongan informasi dengan pengetahuan parametriknya yang tidak terverifikasi. Kombinasi basis pengetahuan yang dikurasi secara manual, kualitas retrieval yang tinggi (NDCG@3 = 0,8100; NDCG@5 = 0,8878), dan *contextual prompting* yang ketat merupakan tiga pilar utama yang menjadikan sistem AudioMatch mampu menekan halusinasi secara signifikan dibandingkan model generatif tanpa augmentasi retrieval.

---

### 4.3.3 Analisis Kualitas Retrieval

Secara umum, hasil pengujian menunjukkan bahwa sistem sangat baik dalam mengurutkan dokumen, artinya dokumen yang paling relevan hampir selalu muncul di urutan teratas, namun masih perlu ditingkatkan dalam hal cakupan, yaitu berapa banyak dokumen relevan yang berhasil ditemukan dari keseluruhan hasil yang ditampilkan. NDCG@3 sebesar 0,8100 dan NDCG@5 sebesar 0,8878 keduanya melampaui target yang ditetapkan dalam Tabel 3.11, yang menunjukkan bahwa sistem *Hybrid Search* berhasil menempatkan dokumen paling relevan di posisi teratas dalam hasil retrieval. Nilai NDCG yang tinggi ini mengindikasikan bahwa penggabungan jalur *vector search* dan BM25 melalui RRF efektif dalam menangani berbagai jenis pertanyaan, mulai dari pertanyaan konseptual hingga pertanyaan yang menyebutkan nama produk secara eksak.

Penggunaan Precision@K di samping NDCG@K didasarkan pada perbedaan mendasar antara kedua metrik yang menjadikannya saling melengkapi. Apabila nilai NDCG rendah, hal tersebut menunjukkan bahwa dokumen relevan memang tersedia dalam basis pengetahuan tetapi sistem gagal menempatkannya di urutan teratas, sehingga konteks terbaik tidak diterima LLM di posisi pertama dan kualitas jawaban terdegradasi. Sebaliknya, apabila nilai Precision rendah sementara NDCG tetap tinggi, sistem berhasil menempatkan dokumen relevan di atas tetapi turut mengikutsertakan banyak dokumen kurang relevan di posisi bawah, sehingga LLM menerima *noise* dalam konteksnya dan risiko jawaban menyimpang tetap ada. Dalam sistem AudioMatch, NDCG@5 sebesar 0,8878 menunjukkan bahwa sistem pandai menempatkan dokumen relevan di posisi teratas, namun Precision@5 sebesar 0,5667 mengindikasikan bahwa rata-rata 2–3 dari 5 dokumen yang masuk ke konteks LLM kurang relevan, terutama pada kategori Kompatibilitas Komponen dengan Precision@5 = 0,3000. Dengan menggunakan kedua metrik secara bersamaan, kelemahan pada dimensi peringkat dan dimensi cakupan dapat diidentifikasi secara terpisah dan spesifik.
...
Sistem AudioMatch berhasil dirancang dan dikembangkan sebagai chatbot konsultasi audio mobil berbasis Retrieval-Augmented Generation (RAG) dan Hybrid Search. Sistem ini dibangun menggunakan FastAPI sebagai framework backend, PostgreSQL dengan pgvector sebagai basis data, VoyageAI voyage-3.5-lite sebagai model embedding, Gemini 2.5 Flash Lite sebagai LLM, dan Upstash Redis sebagai layer caching percakapan. Basis data kendaraan mencakup lebih dari 230 model populer di Indonesia untuk mendukung rekomendasi produk yang kompatibel.

Pengujian fungsionalitas melalui Black Box Testing menghasilkan 9 dari 9 skenario dengan status PASS. Seluruh komponen sistem terbukti berfungsi sesuai spesifikasi yang ditetapkan, mulai dari alur percakapan dasar, deteksi kendaraan, pemeliharaan konteks sesi, validasi input, hingga mekanisme rate limiting. Pengujian juga mengonfirmasi bahwa arsitektur RAG berhasil menekan halusinasi karena sistem secara konsisten memberikan jawaban yang bersumber dari basis pengetahuan terverifikasi dan tidak mengarang informasi di luar domain.

Pengujian kualitas retrieval Hybrid Search menghasilkan NDCG@3 sebesar 0,8100 dan NDCG@5 sebesar 0,8878, keduanya melampaui target yang ditetapkan. Hasil ini menunjukkan bahwa Hybrid Search berhasil menempatkan dokumen paling relevan di posisi teratas. Precision@3 sebesar 0,6445 dan Precision@5 sebesar 0,5667 belum mencapai target, terutama pada kategori Kompatibilitas Komponen yang membutuhkan cakupan basis pengetahuan lebih luas.

Kedua tujuan penelitian terpenuhi. Sistem AudioMatch berhasil menghasilkan respons yang akurat dan minim halusinasi melalui kombinasi basis pengetahuan yang dikurasi secara manual, Hybrid Search berkualitas tinggi, dan contextual prompting yang ketat. Evaluasi teknis menghasilkan gambaran performa yang terukur sekaligus mengidentifikasi arah pengembangan lanjutan, yaitu perluasan basis pengetahuan pada topik teknis spesifik untuk meningkatkan nilai Precision.
...
Pengukuran latensi respons *end-to-end* tidak dilakukan dalam penelitian ini. Evaluasi latensi mencakup tiga komponen utama: waktu proses *Hybrid Search* di basis data, waktu pemanggilan API *embedding* ke VoyageAI, dan waktu generasi respons oleh Gemini. Pengukuran ketiga komponen ini direkomendasikan sebagai arah evaluasi lanjutan guna menilai kelayakan sistem AudioMatch dalam kondisi beban penggunaan nyata.
