...
Secara keseluruhan, AudioMatch dibangun sebagai layanan chatbot berbasis web yang dapat diakses langsung dari browser tanpa memerlukan instalasi perangkat lunak tambahan. Dari sisi teknis, sistem diimplementasikan sebagai aplikasi web berbasis REST API menggunakan framework FastAPI yang di-deploy ke platform Vercel. Basis data menggunakan PostgreSQL yang dilengkapi ekstensi pgvector untuk pencarian vektor dan mekanisme tsvector dengan GIN index untuk full-text search BM25. Caching riwayat percakapan dikelola oleh Upstash Redis melalui antarmuka REST API, yang memastikan kompatibilitas dengan arsitektur serverless Vercel. Model embedding VoyageAI voyage-3.5-lite digunakan untuk menghasilkan representasi vektor 1024 dimensi, sedangkan generasi respons ditangani oleh Google Gemini 2.5 Flash Lite dengan parameter suhu 0,1 untuk menghasilkan keluaran yang deterministik.

Basis pengetahuan AudioMatch merupakan sumber informasi yang menjadi rujukan sistem dalam menjawab pertanyaan pengguna, dan terdiri dari tiga tabel utama yaitu master_customer_problems, master_products, dan master_cars. Tabel master_customer_problems menyimpan pola-pola pertanyaan pelanggan beserta embedding vektor dan kolom tsvector untuk mendukung kedua jalur pencarian dalam Hybrid Search. Tabel master_products memuat 111 produk audio mobil aktif yang mencakup kategori head unit, amplifier, speaker, subwoofer, dan aksesori pengkabelan dari merek-merek yang tersedia di Rendy Audio. Tabel master_cars menyimpan data spesifikasi audio bawaan dari lebih dari 230 model kendaraan populer di Indonesia, meliputi merek, model, tipe, kategori ukuran kabin, tipe dashboard, ukuran speaker bawaan, dan catatan instalasi khusus.

Agar sistem dapat memberikan jawaban yang tepat untuk berbagai jenis pertanyaan — baik yang menyebutkan nama produk secara spesifik maupun yang bersifat konseptual — AudioMatch menggunakan dua cara pencarian informasi secara bersamaan yang kemudian digabungkan hasilnya. Pendekatan ini disebut Hybrid Search, dikonfigurasi dengan bobot 0,6 untuk jalur pencarian berbasis makna kata (vector search) dan 0,4 untuk jalur pencarian berbasis kata kunci persis (BM25). Penggabungan hasil kedua jalur retrieval dilakukan menggunakan algoritma Reciprocal Rank Fusion (RRF) dengan konstanta k = 60. Ambang batas cosine similarity ditetapkan pada 0,3 untuk menyaring dokumen yang tidak relevan secara semantik dari jalur vector search. Apabila tidak ada dokumen yang melampaui ambang batas tersebut, sistem beralih ke jalur product-only fallback yang mencari langsung pada katalog produk melalui fungsi get_products_by_brand() atau Hybrid Search pada tabel master_products.

Untuk memastikan percakapan antara pengguna dan sistem berjalan secara berkesinambungan, setiap sesi percakapan disimpan dan dapat dilanjutkan dalam jangka waktu 24 jam. Secara teknis, setiap sesi diidentifikasi dengan session_id berupa UUID yang dibuat secara otomatis apabila tidak disertakan dalam permintaan. Riwayat percakapan disimpan di Redis dengan nilai TTL 86.400 detik (24 jam), dengan pembatasan maksimal 8 pesan terakhir untuk menjaga efisiensi konteks yang dikirimkan ke Gemini API. Sistem juga dilengkapi dengan mekanisme rate limiting yang membatasi permintaan hingga 100 request per 60 detik untuk menjaga stabilitas layanan.

### 4.1.1 Tampilan Antarmuka AudioMatch
...
### 4.1.2 Implementasi Prompt Engineering
...
**Tabel 4.1** Hasil Black Box Testing AudioMatch
...
| 5 | Session baru tanpa session_id | HTTP 200, session_id baru berupa UUID tersedia dalam respons | HTTP 200, session_id valid (UUID), new_session=True | ✅ PASS |
...
Berdasarkan Tabel 4.1, seluruh 9 skenario pengujian menghasilkan status PASS, sehingga tidak terdapat skenario yang gagal maupun mengalami error.
...
**Tabel 4.2** Ringkasan Hasil Pengujian Retrieval vs Target
...
**Tabel 4.3** Hasil Pengujian Kualitas Retrieval per Kategori Kueri
...
**Tabel 4.4** Detail Hasil Pengujian Kualitas Retrieval per Kueri
...
### 4.3.1 Analisis Black Box Testing
...
### 4.3.2 Analisis Kualitas Retrieval
...
Guna memperjelas perbandingan antara hasil yang dicapai dan target yang ditetapkan, Tabel 4.5 berikut merangkum seluruh metrik evaluasi retrieval dalam format yang siap divisualisasikan sebagai diagram batang.

**Tabel 4.5** Perbandingan Hasil vs Target Pengujian Kualitas Retrieval
...
Pengukuran latensi respons end-to-end sistem, termasuk waktu proses retrieval hybrid, pemanggilan API embedding, dan generasi respons LLM, tidak dilakukan dalam penelitian ini dan direkomendasikan sebagai arah evaluasi lanjutan untuk mengukur kelayakan sistem dalam kondisi penggunaan nyata.
