...
Penelitian ini menggunakan model Waterfall dalam kerangka Software Development Life Cycle (SDLC) sebagai metodologi pengembangan sistem AudioMatch. Model Waterfall dipilih atas tiga pertimbangan yang saling berkaitan.

Pertama, **kebutuhan sistem bersifat stabil dan dapat didefinisikan secara menyeluruh sejak awal**. Arsitektur pipeline RAG, mekanisme Hybrid Search, struktur basis data, spesifikasi API, dan parameter sistem, termasuk ambang batas cosine similarity, bobot RRF, dan ukuran embedding, seluruhnya dapat ditetapkan berdasarkan kebutuhan domain yang sudah diketahui sebelum implementasi dimulai. Tidak ada komponen yang memerlukan eksplorasi desain berulang selama pengembangan berlangsung.

Kedua, **ruang lingkup penelitian terbatas dan terdefinisi secara eksplisit**. Penelitian ini dibatasi pada implementasi sistem backend, yaitu pipeline RAG, komponen Hybrid Search, deteksi kendaraan, dan integrasi layanan eksternal, tanpa mencakup pengembangan antarmuka pengguna yang umumnya memerlukan iterasi desain. Keterbatasan lingkup ini menjadikan pendekatan linier Waterfall lebih efisien daripada pendekatan iteratif seperti Agile, karena tidak ada komponen UI yang memerlukan pengujian pengguna dan penyesuaian desain berulang.

Ketiga, **fokus pada implementasi backend memungkinkan setiap fase menghasilkan output yang konkret dan terverifikasi**. Fase analisis menghasilkan spesifikasi kebutuhan yang langsung menjadi dasar desain skema basis data dan arsitektur API; fase implementasi menghasilkan sistem yang dapat diuji secara langsung melalui Black Box Testing; dan fase pengujian menghasilkan laporan metrik retrieval yang terstandarisasi. Pargaonkar (2023) menegaskan bahwa model Waterfall sangat efektif untuk proyek dengan kebutuhan yang stabil dan terdefinisi dengan baik, sehingga karakteristik AudioMatch menjadikannya metodologi yang tepat untuk penelitian ini.
...
Perlu dijelaskan pula mengapa evaluasi pada fase Pengujian tidak melibatkan pengguna nyata secara langsung. Sistem AudioMatch memang dirancang untuk membantu pelanggan berkonsultasi tentang audio mobil, dan secara ideal pengujian melibatkan pengguna langsung. Namun, pengujian berbasis pengguna baru dapat dilakukan secara bermakna setelah sistem teknis terbukti berfungsi dengan benar. Apabila pipeline RAG atau mekanisme Hybrid Search belum tervalidasi secara teknis, hasil dari pengujian pengguna pun tidak dapat diinterpretasikan dengan tepat karena tidak jelas apakah masalah berasal dari desain sistem atau kegagalan komponen teknis. Oleh karena itu, penelitian ini mendahulukan verifikasi teknis sebagai fondasi sebelum pengujian pengguna dilakukan. Pengujian berbasis pengguna direkomendasikan sebagai tahap lanjutan setelah sistem teknis tervalidasi, sebagaimana diuraikan dalam Bab 5 bagian Saran.
...
Setiap dokumen diverifikasi kesesuaian teknisnya oleh pemilik Rendy Audio sebelum diproses lebih lanjut. Proses kurasi ini merupakan komponen kritis dalam arsitektur RAG AudioMatch: kualitas dan keakuratan basis pengetahuan secara langsung menentukan sejauh mana halusinasi LLM dapat ditekan. Apabila dokumen yang tersedia tidak akurat atau tidak mencakup topik yang ditanyakan, LLM berpotensi mengisi kekosongan informasi tersebut dengan pengetahuan parametriknya yang tidak terverifikasi.

Secara keseluruhan, basis pengetahuan AudioMatch terdiri dari tiga dataset dengan ukuran dan jenis dokumen sebagai berikut. Pertama, **15 entri pola masalah pelanggan** (*knowledge chunks*) pada tabel `master_customer_problems`, yang masing-masing merepresentasikan satu kategori permasalahan audio mobil yang umum diajukan oleh pelanggan Rendy Audio. Kedua, **111 produk audio mobil** pada tabel `master_products`, mencakup delapan kategori produk: head unit single DIN, head unit double DIN, head unit android, amplifier, speaker coaxial, speaker component, subwoofer, dan tweeter, dari 13 merek audio yang tersedia di toko. Ketiga, **230 lebih data kendaraan** pada tabel `master_cars`, meliputi spesifikasi audio bawaan model-model kendaraan populer di pasar Indonesia yang dijadikan dasar filtering kompatibilitas produk.

#### 3.2.4.2 Format Data, Proses Konversi, dan Pembuatan Indeks

Basis pengetahuan AudioMatch menggunakan pendekatan **satu chunk per masalah** untuk tabel `master_customer_problems`. Setiap entri merupakan satu unit *knowledge chunk* yang sudah membawa konteks lengkap berupa judul masalah, deskripsi konteks, daftar kata kunci, dan pendekatan solusi yang direkomendasikan. Pendekatan ini dipilih agar setiap unit retrieval langsung dapat digunakan sebagai dasar rekomendasi produk tanpa perlu penggabungan antar chunk. Format penyimpanan untuk setiap chunk menggunakan tipe data TEXT untuk konten teks, VECTOR(1024) untuk representasi semantik, dan TSVECTOR yang di-*generate* otomatis dari konten teks untuk mendukung pencarian leksikal.

Proses konversi dokumen sumber ke format yang siap digunakan dalam pipeline RAG dilakukan melalui dua tahap. Tahap pertama adalah **chunking dan strukturisasi**: konten dari wawancara, spesifikasi produk, dan data kendaraan distrukturisasi ke dalam format entri basis data yang terdiri dari judul, deskripsi, kata kunci, dan pendekatan solusi. Untuk tabel `master_products`, setiap baris produk secara otomatis menjadi satu unit retrieval yang berisi nama produk, kategori, merek, harga, dan deskripsi teknis. Untuk tabel `master_cars`, setiap baris kendaraan menyimpan spesifikasi audio bawaan yang mencakup tipe dashboard, ukuran speaker OEM, kategori kabin, dan catatan instalasi.

Tahap kedua adalah **indexing**, yang terdiri dari dua proses paralel. Proses pertama adalah pembuatan *embedding* vektor untuk setiap entri menggunakan VoyageAI voyage-3.5-lite melalui endpoint `/api/v1/admin/sync-embeddings`, menghasilkan representasi vektor 1024 dimensi yang disimpan sebagai kolom bertipe `VECTOR(1024)` dengan indeks IVFFlat di PostgreSQL. Proses kedua adalah pembuatan indeks *full-text search* (FTS) menggunakan mekanisme `TSVECTOR` PostgreSQL dengan konfigurasi bahasa Indonesia, yang disimpan sebagai *generated column* `mcp_search_vector` (untuk masalah) dan `mp_search_vector` (untuk produk) dengan GIN index untuk mendukung pencarian BM25 secara efisien.
